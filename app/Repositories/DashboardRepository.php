...
class DashboardRepository
{
    ...
    public function getData(): array
    {
        ...
        $data['totalRegisteredPatientCount'] = User::toBase()->whereType(User::PATIENT)->whereDate('created_at', Carbon::today())->count();
    ...
    {
        $appointments = Appointment::with('services')->whereYear('created_at', Carbon::now()->year)->get();
        $months = [
        ...
        foreach ($months as $month => $monthName) {
            $monthWiseRecords[$monthName] = $appointments
                ->filter(function ($appointment) use ($month) {
                    return (int) $appointment->created_at->month === $month;
                })
